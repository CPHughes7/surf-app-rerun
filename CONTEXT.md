...
Previous session: collapsed the free-product detail view into one full-screen takeover (was a two-step popup-then-sheet flow with a real bug), refreshed the header copy, and made the paid-tier projection meaningfully more honest (a real coastline gate instead of click-anywhere, distance-decay confidence instead of a flat "always low" label, and a "where do people want coverage" signal on the advanced-analysis pitch).

This session: brought a hover-only quick-glance back (Leaflet `Tooltip`, not the old click `Popup` — structurally can't cause the old stacking bug since it's not app state); tightened the private-spot coastline gate from 15 km to 2 km and densified the shoreline outline with the 13 catalog spots' exact coordinates so that's actually achievable; added shoreline-normal auto-facing generation (`estimateFacingDeg`) so admin spot creation is effectively one click, not two; and — the biggest one — extended the same onshore/offshore wind-exposure analysis to the free catalog spots themselves, not just private spots, using their auto-detected shoreline facing. Catalog verdicts can now genuinely differ from before this change (e.g. Holland went from "Marginal" to "Not today" when live wind was blowing straight onshore) — that's intentional and disclosed, not a regression; `computeSurfability`'s actual thresholds are still untouched.
...
- Conditions flow through a source-agnostic seam ([`lib/conditions/resolveConditions.ts`](frontend/src/lib/conditions/resolveConditions.ts) → [`lib/conditions/sources.ts`](frontend/src/lib/conditions/sources.ts)); NOAA is the only source today, fetched once and scored across every spot — then run through [`computeShorelineAwareSurfability`](frontend/src/lib/conditions/projection.ts) ([`useCatalogConditions.ts`](frontend/src/hooks/useCatalogConditions.ts)), which applies the same onshore/offshore wind-exposure adjustment private spots get (facing direction auto-detected from the shoreline outline — [`estimateFacingDeg`](frontend/src/lib/geo/coastline.ts)) while keeping the spot's **real** confidence — no "projected" framing, these are known breaks, not projections. `computeSurfability`'s actual thresholds are untouched
- Hovering a marker shows a quick-glance Leaflet `Tooltip` (name + verdict) — hover-only by construction, so it can't linger or stack with anything; clicking (marker, drawer, or snap) always goes straight to the full-screen detail below
- Drawer ([`LocationsPanel.tsx`](frontend/src/components/LocationsPanel.tsx)) lists catalog spots with live surfability badges
- Detail is a **full-screen takeover** ([`LocationDetailPanel.tsx`](frontend/src/components/LocationDetailPanel.tsx), `position: fixed; inset: 0`, sticky "← Back to map" header, Escape-to-close, background scroll locked) that leads with a **verdict banner** (Go / Marginal / Not today / No verdict yet — copy shared with `/admin` via [`lib/verdictCopy.ts`](frontend/src/lib/verdictCopy.ts)), then an email-capture form, then NOAA + Windy demoted to "the detail, if you want it". Marker click, drawer click, and snap-click all funnel through the same `openDetail()` in `Home.tsx` — there is exactly one way a spot's detail ever opens, so there is never more than one shown at once. No click-triggered popup exists anymore (`LakeMap.tsx` markers have no bound Leaflet `Popup`, only a hover `Tooltip`; the old `PinPopupContent.tsx` is deleted)
...
- Create a spot with one click on a Leaflet map ([`AdminSpotMap.tsx`](frontend/src/components/AdminSpotMap.tsx), same pattern as the public `LakeMap`) — the click sets the location AND auto-generates a facing default from the local shoreline direction ([`estimateFacingDeg`](frontend/src/lib/geo/coastline.ts): nearest outline segment's bearing, rotated 90° clockwise — valid because the outline is wound clockwise, so the lake interior is always on the segment's right-hand side). A second click, if wanted, overrides the facing manually (same bearing math as before). Only the name is ever typed → persisted via `POST /api/private-spots`, which also auto-generates facing server-side if the client omits it
- The click is gated to within **2 km** of a Lake Michigan shoreline outline densified with the 13 catalog spots' exact coordinates ([`lib/geo/coastline.ts`](frontend/src/lib/geo/coastline.ts) rejects early client-side; [`backend/coastline.py`](backend/coastline.py) is the real enforcement in `create_private_spot`, mirrored vertices) — a bad click gets a clear "click closer to shore" message with the actual distance, instead of silently creating a mid-lake or inland spot
