...
- Drawer ends with [`AdvancedAnalysisPitch.tsx`](frontend/src/components/AdvancedAnalysisPitch.tsx) — a distinct, visually-loud CTA ("Point anywhere on the lake. Get a real verdict.") pitching the paid capability specifically. Captures email via the same `notifyMe()` path as per-spot capture, tagged with a reserved `spotId` (`advanced-analysis`) rather than a new table/endpoint — this is the actual demand signal for the paid tier, distinct from "notify me about this one spot"
...
- Leads with two plain counts — **N people interested**, **N want advanced analysis** ([`GET /api/notify-me/stats`](backend/main.py)) — that's the headline; channel breakdown and the recent-signups list are real but tucked behind a single collapsed detail, not the first thing you see. Every `/api/notify-me` submission from `/` still carries first-touch UTM/referrer attribution ([`lib/attribution.ts`](frontend/src/lib/attribution.ts)) if that becomes useful later
